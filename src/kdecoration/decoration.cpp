...
void Decoration::paintBorders(QPainter &p) {
  ...
  p.drawRoundedRect(windowRect, 5, 5);
  p.restore();
}
...
void Decoration::setCornerRadius() {
  const auto client = this->client().lock();
  m_cornerRadius = 5; // TODO ADD CONFIG
}
...
void Decoration::setBorderSizes() {
  ...
  const int left =
      (settings()->borderSize() == KDecoration2::BorderSize::NoSides)
          ? 0
          : borderSize();
...
void Decoration::connectEvents() {
  ...
  connect(clientPtr, &KDecoration2::DecoratedClient::activeChanged, this,
          [this](bool value) { this->update(); });
  connect(settingsPtr, &KDecoration2::DecorationSettings::borderSizeChanged,
